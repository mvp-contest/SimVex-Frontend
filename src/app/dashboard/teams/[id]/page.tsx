...
export default function TeamDetailPage() {
    ...
    const handleCreateProjectSubmit = async () => {
        ...
        try {
            ...
            const newProject = await projectsApi.create(teamId, newProjectName, user.id);

            if (newProjectFiles.length > 0) {
                await projectsApi.uploadFiles(newProject.id, newProjectFiles);
            }
    ...
    const handleProjectFileSelect = (files: Array<{ file: File; url: string; type: string }>) => {
        setNewProjectFiles(files.map((f) => f.file));
    };
    ...
    const handleDeleteTeam = async () => {
        if (!confirm("Are you sure you want to disband this team? This action is irreversible."))
            return;
        try {
    ...
    return (
        <div className="max-w-[1200px] mx-auto p-6 md:p-8">
            ...
            <div className="mb-6">
                ...
                {team.description && (
                    <p className="text-(--color-text-secondary)">{team.description}</p>
                )}
            </div>
                                    ...
                                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium bg-(--color-input-bg) text-(--color-text-muted) overflow-hidden">
                                        {member.user.profile.avatar ? (
                                            <Image
                                                ...
                                                alt={member.user.profile.nickname}
                                                width={32}
                                                height={32}
                                            />
                                        ) : (
                ...
                <div>
                    ...
                    {projects.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed border-(--color-border-primary) rounded-xl bg-(--color-card-bg)/30">
                            ...
                            <p className="text-lg font-medium text-(--color-text-secondary) mb-2">
                                No Projects Yet
                            </p>
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-(--color-text-primary) mb-2">
                                    Project Name{" "}
                                    <span className="text-(--color-status-danger)">*</span>
                                </label>
                                <input
                                    type="text"
                                    ...
                                    className="w-full px-4 py-2.5 bg-(--color-input-bg) border border-(--color-input-border) rounded-lg text-(--color-text-primary) outline-none focus:border-(--color-accent-blue) transition-colors"
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <p className="text-xs text-(--color-text-muted) mb-3">
                                    Supported formats: .glb, .gltf, .obj, .stl, .json
                                </p>
                                <FileUpload onFileSelect={handleProjectFileSelect} />
                                {newProjectFiles.length > 0 && (
                                    <div className="mt-3 p-3 bg-(--color-input-bg) border border-(--color-input-border) rounded-lg">
                                        <p className="text-sm text-(--color-text-secondary) font-medium mb-2">
                                            Selected Files ({newProjectFiles.length}):
                                        </p>
                                        <ul className="space-y-1">
                                            {newProjectFiles.map((file, idx) => (
                                                ...
                                                >
                                                    <span className="w-1.5 h-1.5 rounded-full bg-(--color-accent-blue)" />
                                                    {file.name}
                                                </li>
                                            ))}
                                        </ul>
